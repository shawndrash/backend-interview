...
> [!IMPORTANT]
>
> 一个数据库执行的序列，要么全执行，要么全不执行，要满足ACID
...
> [!IMPORTANT]
>
> 聚集索引：用主键（primary key）组织的B+树结构，储存着表的所有数据
>
> 非聚集索引：不是聚集索引的所有索引都叫非聚集索引，用B+树结构组织，只存索引的数据和主键ID（说白了就是通过自定义索引快速找到主键ID），所以查询时要回表（用ID再查一次），除非查的是索引本身（那tm不是脱裤子放屁么）
>
> 最左前缀原理：实际上源于B+树索引的实现方式。在B+树中，节点按照索引列的顺序从左到右排序。当没有最左列的条件时，无法确定从哪个节点开始查找，因此无法使用索引进行快速定位
